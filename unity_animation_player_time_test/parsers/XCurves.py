...
@timer(name="piecewise_hermite", log_threshold=0.001)
def piecewise_hermite(x_points, y_points, in_slopes, out_slopes, in_weights, out_weights, tangentMode, weightedMode):
    ...
    for seg_start_idx in range(len(indices) - 1):
        ...
        for k in range(num_intervals):
            ...
            out_slope_k = sub_out[k]
            in_slope_k1 = sub_in[k+1]

            use_constant = False
            const_value = None

            # checking if outSlope[k] and inSlope[k+1] are inf
...
@timer(name="_parse_m_Curve", log_threshold=0.001)
...
@timer(name="_parse_curve", log_threshold=0.001)
